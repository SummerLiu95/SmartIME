...
test('preview removes legacy storage and never retains submitted credentials', async () => {
  ...
  await API.saveLLMConfig({ api_key: 'synthetic-test-secret', provider: 'deepseek', model: 'demo' });
  expect(storage.has('smartime_llm')).toBe(false);
...
test('preview scan progress subscription returns a cleanup callback', async () => {
  const unlisten = await API.onRuleScanProgress(() => {});
